...
CHROMECAST_MAC=$(echo "$CHROMECAST_MAC" | tr '[:upper:]' '[:lower:]')
...
while true;
do
    ACTIVE_MACS="$(sudo /usr/sbin/arp-scan --localnet --interface=$IFACE --quiet --ignoredups  | tail -n +3 | head -n -3 | cut -f2)"
    CHROMECAST="0"
    ISEMPT="1"
