...
for i in $( ls $SMARTHOME_DIR/modules/roomba_520/cron/ ); do
 ...
 if [ -f "$SMARTHOME_DIR/smartcron/ROOMBA_$i" ] ; then
   $SMARTHOME_DIR/bin/memdb_client.py $SMARTHOME_MEMDB_PORT S PIDS $MODULENAME "${i/ROOMBA_/}" "-1" > /dev/null 
 else
   $SMARTHOME_DIR/bin/memdb_client.py $SMARTHOME_MEMDB_PORT S PIDS $MODULENAME "${i/ROOMBA_/}" "0" > /dev/null
 fi
done
...
do
   for i in $( ls $SMARTHOME_DIR/modules/$MODULENAME/cron/  ); do
       STATUS=$($SMARTHOME_DIR/bin/memdb_client.py $SMARTHOME_MEMDB_PORT G PIDS $MODULENAME $i 2> /dev/null )
